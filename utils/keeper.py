...
class Keeper(object):
    ...
    def save_img(self, epoch, step, plot_obj, img_name='test.jpg'):
        ...
        img_file = os.path.join(img_path, str(epoch)+str(step)+img_name)
        plt.subplot(221)
        plt.imshow(plot_obj[0])
        plt.subplot(222)
        plt.imshow(plot_obj[1][:, :, 0], cmap='gray')
        plt.subplot(223)
        plt.imshow(plot_obj[2][:, :, 0], cmap='gray')
